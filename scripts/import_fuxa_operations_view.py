"""Import or update the Topic 127 Operations Overview view in FUXA."""
...
def load_view(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise FileNotFoundError(
            f"FUXA Operations Overview view not found: {path}"
        )
    ...
    if view.get("name") != "Operations Overview":
        raise ValueError(
            f"Unexpected view name: {view.get('name')!r}"
        )

    if not view.get("svgcontent"):
        raise ValueError(
            "Operations Overview view has no SVG content."
        )
...
def main() -> int:
    ...
    if VIEW_ID not in view_ids:
        raise RuntimeError(
            "Operations Overview view was not persisted by FUXA."
        )
    ...
    if len(operations_views) != 1:
        raise RuntimeError(
            "Expected exactly one Operations Overview view."
        )

    ledger_views = [
        item
        for item in updated_views
        if item.get("id") == LEDGER_VIEW_ID
    ]

    if len(ledger_views) != 1:
        raise RuntimeError(
            "Existing Simulated Ledger Health view was not preserved."
        )
    ...
    print()
    print(
        "FUXA Operations Overview import: PASS"
    )
