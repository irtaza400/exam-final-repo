...
| 33 | Deployment                  | Terraform Option B / AWS EC2 / Ubuntu | `terraform/main.tf`, `terraform/network.tf`, `terraform/ec2.tf`, `terraform/security_group.tf`, `terraform/user_data.tftpl` | Terraform + EC2 deployment output          | A02, A06                | Terraform plan/apply evidence + EC2 environment + Docker Compose status | IMPLEMENTED / CAPTURE PENDING |
